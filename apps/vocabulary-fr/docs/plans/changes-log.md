...
- 2026-08-22 **文法レビュー完了（6スライス独立並列・全2486文）**: 天井違反10件（比較級/最上級6件・受動態2件・時制1件・形容詞位置1件）、翻訳/語義2件、性の不一致1件、正書法1件、見出し語未実現1件、動詞用法1件、**敬体(です/ます)混入21件**を検出・修正。
  - 比較級/最上級の自己使用例外を精緻化: `que`を使った明示比較と`le/la`最上級は禁止のまま、`pire`（`mauvais`の不規則比較級で非比較の語義を持たない）を裸述語のみ許可する形で例外表に追加。
  - **敬体混入は21件と全スライスの合計で見ると系統的** — 命令文（〜てください）と疑問文（〜ですか）に集中。プレーン体（て形単独・か抜きの疑問形）に統一。
  - 両ゲート再検証: `validate_data.mjs` ✓1218語 / `check_vocab.py` ✓0違反 / `node --test` 90/90 green。敬体スイープ再実行で残存0件を確認。
- 2026-08-22 **`œ`合字の綴り誤りを修正**: `boeuf/coeur/soeur/oeuf/oeuvre/oeil` が正書法上の `bœuf/cœur/sœur/œuf/œuvre/œil` になっていなかった（FLELex/Lexique383 のソースが `oe` 表記を使うため）。見出し語・複数形・全例文中の使用箇所（11文）を修正。ID は変わらない（`slug()` が合字を `oe` に展開してから正規化するため）。
- 2026-08-23 **A1 完成**: 1,218語 / 例文2,486文 / 音声3,704クリップ（見出し語1,218＋例文2,486・fr-FR-DeniseNeural・カラオケtiming付き）。
  - 検証: スキーマ ✓1218語（音声必須）／ **スパイラル語彙ゲート ✓0違反** ／ **文法天井の独立レビュー（6エージェントで全2486文）→ 天井違反10件・翻訳/性/正書法4件・敬体混入21件を検出、全て修正して再検証0件**。
  - UI確認: フラッシュカード（同綴り異品詞は3例文）・例文カラオケ再生とハイライト・読むだけ1218語＋検索・進捗、console error 0。BMWデザイン正本を継承（css/styles.css は蘭語版と同一）。
  - 音声生成中に**Macスリープでプロセスが34時間停止**する事故が発生。`caffeinate -i` を付けて再起動し解決（今後の音声生成では標準手順にすること）。
  - 独語・蘭語アプリへの変更は0件。
