...
- **既知の限界:** 小モデルは短文で誤 lemma を返すことがある（例: "eten broden" の `broden` を動詞と誤判定）。`check_vocab.py` は「lemma・表層形・機能語」の三経路で照合し、なお外れた語を**違反候補**として列挙する。偽陽性は (a) `function_words_nl.txt` 追補、(b) 正当な派生語を見出し語 seed に追加、(c) それでも残るものは changes-log に理由記録、で運用。精度が要れば `nl_core_news_md` へ上げる。
